 ...
 * @since  1.6
 */
class MoviesViewTests extends JViewLegacy
{
	protected $items;
	...
	protected function addToolbar()
	{
		...
		JToolBarHelper::title(JText::_('COM_MOVIES_TITLE_TESTS'), 'tests.png');

		// Check if the form exists before showing the add/edit buttons
		$formPath = JPATH_COMPONENT_ADMINISTRATOR . '/views/test';
		...
		if (file_exists($formPath))
		{
			if ($canDo->get('core.create'))
			{
				...
				{
					JToolbarHelper::custom('tests.duplicate', 'copy.png', 'copy_f2.png', 'JTOOLBAR_DUPLICATE', true);
				}
			}
		...
		if ($canDo->get('core.edit.state'))
		{
			...
			{
				JToolBarHelper::divider();
				JToolBarHelper::archiveList('tests.archive', 'JTOOLBAR_ARCHIVE');
			}

			if (isset($this->items[0]->checked_out))
			{
				JToolBarHelper::custom('tests.checkin', 'checkin.png', 'checkin_f2.png', 'JTOOLBAR_CHECKIN', true);
			}
		}
		...
		JHtmlSidebar::setAction('index.php?option=com_movies&view=tests');
	}
