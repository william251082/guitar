 ...
 * @since  1.6
 */
class GuitarModelTransaction extends JModelAdmin
{
	/**
	 ...
	 * @since   3.2
	 */
	public $typeAlias = 'com_guitar.transaction';
	...
	public function getForm($data = array(), $loadData = true)
	{
            ...
            // Get the form.
            $form = $this->loadForm(
                    'com_guitar.transaction', 'transaction',
                    array('control' => 'jform',
                            'load_data' => $loadData
                    )
            );
	...
	protected function loadFormData()
	{
		...
		$data = JFactory::getApplication()->getUserState('com_guitar.edit.transaction.data', array());
	...
	/**
	 * Method to duplicate an Transaction
	 *
		...
		foreach ($pks as $pk)
		{
			...
			if ($table->load($pk, true))
			{
				...
				if (!empty($table->place))
				{
					...
					{
						$table->place = implode(',', $table->place);
					}
				}
				else
				{
	...
	protected function prepareTable($table)
	{
		...
		if (empty($table->id))
		{
			...
			{
				$db = JFactory::getDbo();
				$db->setQuery('SELECT MAX(ordering) FROM #__guitar_transactions');
				$max             = $db->loadResult();
