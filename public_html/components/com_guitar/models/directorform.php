 ...
 * @since  1.6
 */
class GuitarModelDirectorForm extends JModelForm
{
    private $item = null;
    ...
    protected function populateState()
    {
        ...
        {
                $id = Factory::getApplication()->getUserState('com_guitar.edit.director.id');
        }
        else
        {
                $id = Factory::getApplication()->input->get('id');
                Factory::getApplication()->setUserState('com_guitar.edit.director.id', $id);
        }
    ...
    public function getItem($id = null)
    {
        if ($this->item === null)
        {
            ...
            if ($table !== false && $table->load($id))
            {
				...
				if ($id)
				{
					$canEdit = $user->authorise('core.edit', 'com_guitar.director.' . $id) || $user->authorise('core.create', 'com_guitar.director.' . $id);
				}
				else
				{
                ...
                if (!$canEdit && $user->authorise('core.edit.own', 'com_guitar.director.' . $id))
                {
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_guitar.director', 'directorform', array(
                        'control'   => 'jform',
                        'load_data' => $loadData
                )
        );
    ...
    {
        $data = Factory::getApplication()->getUserState('com_guitar.edit.director.data', array());
    ...
    public function save($data)
    {
        ...
        if ($id)
        {
            ...
            $authorised = $user->authorise('core.edit', 'com_guitar.director.' . $id) || $authorised = $user->authorise('core.edit.own', 'com_guitar.director.' . $id);
        }
        else
        {
    ...
    public function delete($pk)
    {
        ...
        if(!$pk || $this->userIDItem($pk) || $this->isAdminOrSuperUser()){
            ...
            if ($user->authorise('core.delete', 'com_guitar.director.' . $id) !== true)
            {
                    throw new Exception(JText::_('JERROR_ALERTNOAUTHOR'), 403);
            }
