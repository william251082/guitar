 ...
 * @since  1.6
 */
class GuitarModelGuitaristForm extends JModelForm
{
    private $item = null;
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_guitar.guitarist', 'guitaristform', array(
                        'control'   => 'jform',
                        'load_data' => $loadData
                )
        );
    ...
    {
        $data = Factory::getApplication()->getUserState('com_guitar.edit.guitarist.data', array());
    ...
    {
        $id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('guitarist.id');
    ...
    public function delete($pk)
    {
        ...
        if(!$pk || $this->userIDItem($pk) || $this->isAdminOrSuperUser()){
            if (empty($pk))
            {
                    $pk = (int) $this->getState('guitarist.id');
            }
