 ...
 * @since  1.6
 */
class GuitarModelSongForm extends JModelForm
{
    private $item = null;
    ...
    protected function populateState()
    {
        ...
        {
                $id = Factory::getApplication()->getUserState('com_guitar.edit.song.id');
        }
        else
        {
                $id = Factory::getApplication()->input->get('id');
                Factory::getApplication()->setUserState('com_guitar.edit.song.id', $id);
        }
    ...
    public function getItem($id = null)
    {
        if ($this->item === null)
        {
            ...
            if ($table !== false && $table->load($id))
            {
				...
				if ($id)
				{
					$canEdit = $user->authorise('core.edit', 'com_guitar.song.' . $id) || $user->authorise('core.create', 'com_guitar.song.' . $id);
				}
				else
				{
                ...
                if (!$canEdit && $user->authorise('core.edit.own', 'com_guitar.song.' . $id))
                {
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_guitar.song', 'songform', array(
                        'control'   => 'jform',
                        'load_data' => $loadData
                )
        );
    ...
    {
        $data = Factory::getApplication()->getUserState('com_guitar.edit.song.data', array());
    ...
    {
        $id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('song.id');
        ...
        if ($id)
        {
            ...
            $authorised = $user->authorise('core.edit', 'com_guitar.song.' . $id) || $authorised = $user->authorise('core.edit.own', 'com_guitar.song.' . $id);
        }
        else
        {
    ...
    public function delete($pk)
    {
        ...
        if(!$pk || $this->userIDItem($pk) || $this->isAdminOrSuperUser()){
            ...
            if ($user->authorise('core.delete', 'com_guitar.song.' . $id) !== true)
            {
                    throw new Exception(JText::_('JERROR_ALERTNOAUTHOR'), 403);
            }
