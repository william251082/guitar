 ...
 * @since  1.6
 */
class GuitarModelGenreForm extends JModelForm
{
    private $item = null;
    ...
    protected function populateState()
    {
        ...
        {
                $id = Factory::getApplication()->getUserState('com_guitar.edit.genre.id');
        }
        else
        {
                $id = Factory::getApplication()->input->get('id');
                Factory::getApplication()->setUserState('com_guitar.edit.genre.id', $id);
        }

        $this->setState('genre.id', $id);
        ...
        {
                $this->setState('genre.id', $params_array['item_id']);
        }
     ...
     */
    public function getTable($type = 'Genre', $prefix = 'GuitarTable', $config = array())
    {
    ...
    {
        // Get the id.
        $id = (!empty($id)) ? $id : (int) $this->getState('genre.id');
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_guitar.genre', 'genreform', array(
                        'control'   => 'jform',
                        'load_data' => $loadData
                )
        );
    ...
    {
        $data = Factory::getApplication()->getUserState('com_guitar.edit.genre.data', array());
    ...
    {
        $id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('genre.id');
