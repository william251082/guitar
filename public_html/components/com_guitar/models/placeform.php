 ...
 * @since  1.6
 */
class GuitarModelPlaceForm extends JModelForm
{
    private $item = null;
    ...
    public function getItem($id = null)
    {
        if ($this->item === null)
        {
            ...
            if ($table !== false && $table->load($id))
            {
				...
				if ($id)
				{
					$canEdit = $user->authorise('core.edit', 'com_guitar.place.' . $id) || $user->authorise('core.create', 'com_guitar.place.' . $id);
				}
				else
				{
                ...
                if (!$canEdit && $user->authorise('core.edit.own', 'com_guitar.place.' . $id))
                {
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_guitar.place', 'placeform', array(
                        'control'   => 'jform',
                        'load_data' => $loadData
                )
        );
    ...
    {
        $data = Factory::getApplication()->getUserState('com_guitar.edit.place.data', array());
    ...
    public function save($data)
    {
        ...
        if ($id)
        {
            ...
            $authorised = $user->authorise('core.edit', 'com_guitar.place.' . $id) || $authorised = $user->authorise('core.edit.own', 'com_guitar.place.' . $id);
        }
        else
        {
    ...
    public function delete($pk)
    {
        ...
        if(!$pk || $this->userIDItem($pk) || $this->isAdminOrSuperUser()){
            ...
            if ($user->authorise('core.delete', 'com_guitar.place.' . $id) !== true)
            {
                    throw new Exception(JText::_('JERROR_ALERTNOAUTHOR'), 403);
            }
