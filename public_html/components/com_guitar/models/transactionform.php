 ...
 * @since  1.6
 */
class GuitarModelTransactionForm extends JModelForm
{
    private $item = null;
    ...
    public function getItem($id = null)
    {
        if ($this->item === null)
        {
            ...
            if ($table !== false && $table->load($id))
            {
				...
				if ($id)
				{
					$canEdit = $user->authorise('core.edit', 'com_guitar.transaction.' . $id) || $user->authorise('core.create', 'com_guitar.transaction.' . $id);
				}
				else
				{
                ...
                if (!$canEdit && $user->authorise('core.edit.own', 'com_guitar.transaction.' . $id))
                {
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_guitar.transaction', 'transactionform', array(
                        'control'   => 'jform',
                        'load_data' => $loadData
                )
        );
    ...
    {
        $data = Factory::getApplication()->getUserState('com_guitar.edit.transaction.data', array());
    ...
    public function save($data)
    {
        ...
        if ($id)
        {
            ...
            $authorised = $user->authorise('core.edit', 'com_guitar.transaction.' . $id) || $authorised = $user->authorise('core.edit.own', 'com_guitar.transaction.' . $id);
        }
        else
        {
    ...
    public function delete($pk)
    {
        ...
        if(!$pk || $this->userIDItem($pk) || $this->isAdminOrSuperUser()){
            ...
            if ($user->authorise('core.delete', 'com_guitar.transaction.' . $id) !== true)
            {
                    throw new Exception(JText::_('JERROR_ALERTNOAUTHOR'), 403);
            }
