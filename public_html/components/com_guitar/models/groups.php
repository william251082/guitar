 ...
 * @since  1.6
 */
class GuitarModelGroups extends JModelList
{
	/**
	 * Constructor.
	 *
	...
	protected function getListQuery()
	{
            ...
            $query->from('`#__guitar_groups` AS a');
		...
		// Filtering place
		$filter_place = $this->state->get("filter.place");

		if ($filter_place)
		{
			$query->where("a.`place` = '".$db->escape($filter_place)."'");
		}
		...
		foreach ($items as $item)
		{
			...
			if (isset($item->transaction))
			{
				...
				foreach ($values as $value)
				{
					...
					$query = $db->getQuery(true);
					$query
						->select('`#__guitar_transactions_3044863`.`title`')
						->from($db->quoteName('#__guitar_transactions', '#__guitar_transactions_3044863'))
			...
			if (isset($item->place))
			{
				...
				$item->place = !empty($textValue) ? implode(', ', $textValue) : $item->place;
			}
