...
class JHtmlIcon
{
    public static function email($item, $params, $attribs = array())
    {
        ...
        $link	= $base.JRoute::_('index.php?option=com_guitar&view=song&catid='.$item->catslug.'&id='.$item->slug, false);
    ...
    public static function print_popup($item, $params = NULL, $attribs = array())
    {
        $uri	= JURI::getInstance();
        $base	= $uri->toString(array('scheme', 'host', 'port'));
        $url  = 'index.php?option=com_guitar&view=song&catid='.$item->catslug.'&id='.$item->slug.'&tmpl=component&print=1&layout=default';
