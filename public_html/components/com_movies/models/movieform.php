 ...
 * @since  1.6
 */
class MoviesModelMovieForm extends JModelForm
{
    private $item = null;
        ...
        */
        ...
        public function userIDItem($id){
            try{
                ...
                $query->select("id")
                      ->from($db->quoteName('#__movies_movies'))
    ...
    protected function populateState()
    {
        ...
        {
                $id = Factory::getApplication()->getUserState('com_movies.edit.movie.id');
        }
        else
        {
                $id = Factory::getApplication()->input->get('id');
                Factory::getApplication()->setUserState('com_movies.edit.movie.id', $id);
        }
    ...
    public function getItem($id = null)
    {
        if ($this->item === null)
        {
            ...
            if ($table !== false && $table->load($id))
            {
				...
				if ($id)
				{
					$canEdit = $user->authorise('core.edit', 'com_movies.movie.' . $id) || $user->authorise('core.create', 'com_movies.movie.' . $id);
				}
				else
				{
                ...
                if (!$canEdit && $user->authorise('core.edit.own', 'com_movies.movie.' . $id))
                {
     ...
     */
    public function getTable($type = 'Movie', $prefix = 'MoviesTable', $config = array())
    {
    ...
    {
        // Get the form.
        $form = $this->loadForm('com_movies.movie', 'movieform', array(
                        'control'   => 'jform',
                        'load_data' => $loadData
                )
        );
    ...
    {
        $data = Factory::getApplication()->getUserState('com_movies.edit.movie.data', array());
    ...
    {
        $id    = (!empty($data['id'])) ? $data['id'] : (int) $this->getState('movie.id');
        ...
        if ($id)
        {
            ...
            $authorised = $user->authorise('core.edit', 'com_movies.movie.' . $id) || $authorised = $user->authorise('core.edit.own', 'com_movies.movie.' . $id);
        }
        else
        {
    ...
    public function delete($pk)
    {
        ...
        if(!$pk || $this->userIDItem($pk) || $this->isAdminOrSuperUser()){
            ...
            if ($user->authorise('core.delete', 'com_movies.movie.' . $id) !== true)
            {
                    throw new Exception(JText::_('JERROR_ALERTNOAUTHOR'), 403);
            }
